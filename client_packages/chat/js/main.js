...
let timeStamps = (timeStamp) => { //currentHealth - value, what we send from server.
    timestamp :true;
};
...
const getDateString = () => {
    const date = new Date();
    const h = "0"+date.getUTCHours().toString();
    const m = "0"+date.getUTCMinutes().toString();
    const s = "0"+date.getUTCSeconds().toString();
